...
class default_options_form extends moodleform {
    ...
    public function definition() {
        ...
        if(has_capability('local/culrollover:csvupload', context_system::instance())) {
            $mform->addElement('header', 'csvupload', get_string('csvupload_header', 'local_culrollover'));
            $mform->setExpanded('csvupload', false);
            $mform->addElement('filepicker', 'csvfile', get_string('file'), null, array('maxbytes' => '1048576', 'accepted_types' => array('*.csv')));
            $mform->addHelpButton('csvfile', 'csvfile_helptext', 'local_culrollover');
        }
