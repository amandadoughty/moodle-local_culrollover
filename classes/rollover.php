...
class rollover {
    ...
    public function copy_courses() {
        ...
        $this->delete_turnitin_activities();
    ...
    private function delete_turnitin_activities() {
        global $DB, $CFG ;
        ...
        if($module = $DB->get_record('modules', array('name' => 'turnitintooltwo'))) {
            ...
            foreach($plagiarismsettings as $plagiarismsetting) {
                if(!isset($assignswithtii[$plagiarismsetting->cm])) {
     ...
     * 
     */
     private function update_course_settings() {
        global $DB, $CFG;
        ...
        if ($this->record->merge == 0) {
            ...
            $this->copy_course_section_options();
        }
     ...
     * @param $destid
     */
    private function copy_course_section_options() {
        global $DB;
        ...
        if ($dstoption = $DB->get_record('course_format_options', $dstoption)) {
            $dstoption->value = $srcoptions->value;
            $DB->update_record('course_format_options', $dstoption);
        } else {
            $dstoption->value = $srcoptions->value;
            $DB->insert_record('course_format_options', $dstoption);
        }
    }    
