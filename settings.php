...
if ($hassiteconfig) {
	...
	$settings->add(new admin_setting_configtext(
		'src_filter_term', 
		get_string('src_identifier', 'local_culrollover'),    
		get_string('src_identifier_helper', 'local_culrollover'), 
		'2013',    
		PARAM_ALPHANUM
		)
	);
	...
	$settings->add(new admin_setting_configtext(
		'dest_filter_term', 
		get_string('dest_identifier', 'local_culrollover'),   
		get_string('dest_identifier_helper', 'local_culrollover'), 
		'2014',    
		PARAM_ALPHANUM
		)
	);
	...
	// Check if Turnitintool Two is installed and then show the necessary setting.
	if($DB->record_exists('modules', array('name' => 'turnitintooltwo'))) {
	    $settings->add(new admin_setting_configcheckbox(
	    	'delete_turnitinv2',
	    	get_string('delete_turnitin_v2', 'local_culrollover'),
	    	get_string('delete_turnitin_v2_desc', 'local_culrollover'),
	    	0
	    	)
	    );
	}
